...
def get_entropy_weight(epoch):
    # if epoch > EPOCH:
    #     return 0.1 / math.sqrt(epoch - EPOCH)
    # else:
...
class A3C:
    ...
    def train(self, s_batch, a_batch, r_batch, end_of_video, epoch):
        ...
        # print(type(s_batch), type(a_batch), type(r_batch))
        ...
        self.td_loss_list.append(torch.mean(td_batch ** 2))
    ...
    def update(self):
        ...
        if self.is_central:
            ...
            self.critic_optim.zero_grad()
            self.td_loss = np.mean(self.td_loss_list)
            self.td_loss_list = []
